...
- **ImportBatch** — one per uploaded statement, with parsed metadata (cardholder, card number, limits, statement/payment dates).
...
- When aggregating money, group by `currency` (a user can have GTQ and USD expenses); dashboard KPIs report `byCurrency` totals.
